    ...
    while oheap:
        ...
        for i, j in neighbors:
            ...
            if not (0 <= neighbor[1] < array.shape[1]):
                continue
            current_c = array[current[0], current[1]]
            neighbor_c = array[neighbor[0], neighbor[1]]
            if ord(neighbor_c) - ord(current_c) > 1:
                continue
            if (neighbor in closed and
                    gscore >= gscores.get(neighbor, 0)):
                continue
